...
from scripts.phase4ah_controlled_extension_fetch import (  # noqa: E402
    READY,
    RAW_OUTPUT_DIR,
    SUMMARY_PATH,
...
JSON_REPORT_PATH = Path("reports/phase_reports/phase4ah_controlled_extension_fetch_audit.json")
MARKDOWN_REPORT_PATH = Path("docs/phase_reports/phase4ah_controlled_extension_fetch_audit.md")
...
def run_audit(
    *,
    ...
    if not summary:
        summary = run_controlled_extension_fetch(summary_path=summary_path, raw_output_dir=raw_output_dir)
    ...
    checks = {
        ...
        "phase4ag_summary_detected": summary.get("phase4ag_summary_detected") is True
        ...
        or summary.get("readiness_status") in {"BLOCKED_BY_MISSING_CREDENTIAL", "BLOCKED_BY_MISSING_DRY_RUN_REQUESTS"},
        ...
        "extension_fetch_status_consistent": isinstance(summary.get("extension_fetch_executed"), bool),
        "raw_manifest_exists_when_fetch_attempted": (
            not summary.get("extension_fetch_executed") or run_manifest_path.is_file()
        ),
        "request_manifests_exist_when_success_or_failure": (
            not summary.get("extension_fetch_executed")
            or bool(request_manifests)
            or int(summary.get("skipped_extension_request_count") or 0)
            == int(summary.get("planned_extension_request_count") or 0)
        ),
        "responses_exist_when_success": (
            int(summary.get("succeeded_extension_request_count") or 0) == 0 or bool(response_files)
        ),
        ...
        "existing_success_manifest_preserved": summary.get("existing_success_manifest_preserved") is True,
    ...
    result = {
        ...
        "pytest_hint": "python3 -m pytest tests/test_phase4ah_controlled_extension_fetch.py && python3 -m pytest -q",
    }
...
def _compact_summary(summary: dict[str, Any]) -> dict[str, Any]:
    keys = (
        ...
        "planned_extension_request_count",
        "executed_extension_request_count",
        "succeeded_extension_request_count",
        "failed_extension_request_count",
        "skipped_extension_request_count",
        "pagination_request_count",
        "fetched_extension_date_min",
        "fetched_extension_date_max",
        "fetched_extension_date_count",
        "fetched_extension_row_count",
        "fetched_extension_code_count",
