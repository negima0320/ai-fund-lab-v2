...
PHASE = "Phase4-AZ"
PHASE4AY_SUMMARY_PATH = Path("reports/candidate_ai/full_range/phase4ay_long_history_request_regeneration_summary.json")
PHASE4AY_REQUESTS_PATH = Path("reports/candidate_ai/full_range/phase4ay_long_history_corrected_requests.json")
SUMMARY_PATH = Path("reports/candidate_ai/full_range/phase4az_long_history_controlled_fetch_retry_summary.json")
REPORT_PATH = Path("docs/phase_reports/phase4az_long_history_controlled_fetch_retry.md")

READY = "READY_FOR_LONG_HISTORY_RAW_COVERAGE_AUDIT"
PARTIAL_READY = "PARTIAL_FETCH_READY_FOR_RESUME"
BLOCKED_MISSING_AY = "BLOCKED_BY_MISSING_PHASE4AY_SUMMARY"
BLOCKED_AY_NOT_READY = "BLOCKED_BY_PHASE4AY_NOT_READY"
BLOCKED_MISSING_REQUESTS = "BLOCKED_BY_MISSING_CORRECTED_REQUESTS"
BLOCKED_PRE_START = "BLOCKED_BY_PRE_START_REQUEST_PRESENT"
...
CORRECTED_START_DATE = "2021-06-01"
...
def run_long_history_controlled_fetch_retry(
    *,
    phase4ay_summary_path: Path = PHASE4AY_SUMMARY_PATH,
    phase4ay_requests_path: Path = PHASE4AY_REQUESTS_PATH,
    ...
    phase4ay = _read_json_optional(phase4ay_summary_path)
    if not phase4ay:
        summary = _blocked_summary(summary_path, BLOCKED_MISSING_AY)
        _write_markdown_report(report_path, summary)
        return summary
    if phase4ay.get("readiness_status") != "READY_FOR_LONG_HISTORY_CONTROLLED_FETCH_RETRY":
        summary = _blocked_summary(summary_path, BLOCKED_AY_NOT_READY, phase4ay=phase4ay)
        _write_markdown_report(report_path, summary)
        return summary
    artifact = _read_json_optional(phase4ay_requests_path)
    requests = artifact.get("requests") if isinstance(artifact.get("requests"), list) else []
    if not requests:
        summary = _blocked_summary(summary_path, BLOCKED_MISSING_REQUESTS, phase4ay=phase4ay)
        _write_markdown_report(report_path, summary)
        return summary
    pre_start_requests = [request for request in requests if _request_date(request) < CORRECTED_START_DATE]
    if pre_start_requests:
        summary = _blocked_summary(
            summary_path,
            BLOCKED_PRE_START,
            phase4ay=phase4ay,
            corrected_request_count=len(requests),
            pre_start_request_count=len(pre_start_requests),
        )
        _write_markdown_report(report_path, summary)
        return summary
    if not _output_paths_safe(raw_output_dir):
        summary = _blocked_summary(summary_path, BLOCKED_OUTPUT_PATH, phase4ay=phase4ay, corrected_request_count=len(requests))
        _write_markdown_report(report_path, summary)
        return summary
    if max_pages < 1:
        summary = _blocked_summary(summary_path, BLOCKED_RATE_LIMIT, phase4ay=phase4ay, corrected_request_count=len(requests))
        _write_markdown_report(report_path, summary)
        return summary
    ...
    try:
        ...
        return summary
    common_policy = (
        client.common_policy_manifest(ENDPOINT)
        if hasattr(client, "common_policy_manifest")
        else {
            "rate_limit_policy": {"max_requests_per_minute": getattr(settings.jquants, "rate_limit_per_minute", 60)},
            "retry_policy": {"max_attempts": 2},
            "endpoint_capability": {"endpoint": ENDPOINT},
        }
    )
    ...
    quarantined_failed_manifest_count = _count_pre_start_failed_manifests(request_manifest_dir)
    ...
    for planned_request in requests:
        ...
        if not target_date:
            ...
            continue
        if target_date < CORRECTED_START_DATE:
            summary = _blocked_summary(
                summary_path,
                BLOCKED_PRE_START,
                phase4ay=phase4ay,
                corrected_request_count=len(requests),
                pre_start_request_count=1,
                credential_read_performed=credential_read_performed,
                http_client_initialized=True,
                secret_present=secret_present,
            )
            summary["pre_start_request_executed"] = False
            _write_markdown_report(report_path, summary)
            return summary
    ...
    run_manifest = _run_manifest(
        ...
        secret_present=secret_present,
        quarantined_failed_manifest_count=quarantined_failed_manifest_count,
    ...
    summary = {
        ...
        "phase4ay_summary_detected": True,
        "phase4ay_readiness_status": phase4ay.get("readiness_status"),
        "corrected_requests_detected": True,
        "corrected_fetch_start_date": phase4ay.get("corrected_fetch_start_date"),
        "fetch_end_date": phase4ay.get("fetch_end_date"),
        "corrected_request_count": len(requests),
        ...
        "pre_start_request_executed": False,
        "quarantined_failed_manifest_count": quarantined_failed_manifest_count,
        "endpoint": ENDPOINT,
        "method": "GET",
...
def _request_manifest(*, target_date: str, status: str, page_count: int, row_count: int, error_message: str | None) -> dict[str, Any]:
    return {
        "created_at": _now_utc(),
        "phase": PHASE,
        "endpoint": ENDPOINT,
...
def _run_manifest(
    *,
    ...
    raw_output_dir: Path,
    secret_present: bool,
    quarantined_failed_manifest_count: int,
    ...
    return {
        ...
        "corrected_fetch_start_date": phase4ay.get("corrected_fetch_start_date"),
        "fetch_end_date": phase4ay.get("fetch_end_date"),
        "corrected_request_count": corrected_request_count,
        "executed_request_count": executed,
        "succeeded_request_count": succeeded,
        "failed_request_count": failed,
        "skipped_request_count": skipped,
...
def _blocked_summary(
    ...
    *,
    phase4ay: dict[str, Any] | None = None,
    corrected_request_count: int = 0,
    pre_start_request_count: int = 0,
    credential_read_performed: bool = False,
    http_client_initialized: bool = False,
    secret_present: bool = False,
    error_message: str | None = None,
) -> dict[str, Any]:
    phase4ay = phase4ay or {}
    summary = {
        "phase": PHASE,
        "status": "BLOCKED",
        "readiness_status": readiness_status,
        ...
        "promotion_performed": False,
        "reader_switch_performed": False,
        "phase4ay_summary_detected": bool(phase4ay),
        "phase4ay_readiness_status": phase4ay.get("readiness_status"),
        "corrected_requests_detected": corrected_request_count > 0,
        "corrected_fetch_start_date": phase4ay.get("corrected_fetch_start_date"),
        "fetch_end_date": phase4ay.get("fetch_end_date"),
        "corrected_request_count": corrected_request_count or int(phase4ay.get("request_count") or 0),
        ...
        "pre_start_request_executed": False,
        "pre_start_request_count": pre_start_request_count,
        "quarantined_failed_manifest_count": 0,
        "endpoint": ENDPOINT,
        "method": "GET",
        "rate_limit_policy": "J-Quants Light plan: 60 req/min.",
        "raw_output_path": str(RAW_OUTPUT_DIR) + "/",
        "raw_manifest_path": str(RAW_OUTPUT_DIR / "manifest.json"),
        "secret_present": secret_present,
        "secret_value_logged": False,
        ...
        "error_message": error_message,
        "recommended_next_action": "Resolve the blocking condition before Phase4-AZ retry.",
    }
    _write_json(summary_path, summary)
    return summary
...
def _recommended_next_action(readiness_status: str) -> str:
    if readiness_status == READY:
        return "Phase4-BA Long History Raw Coverage Audit before normalization."
    if readiness_status == PARTIAL_READY:
        return "Rerun Phase4-AZ to resume failed or missing corrected requests before Phase4-BA."
...
def _count_pre_start_failed_manifests(manifest_dir: Path) -> int:
    count = 0
...
def _sanitize_text(text: str, secret: str | None = None) -> str:
    ...
    for marker in ("Authorization", "x-api-key", "JQUANTS_API_KEY", "password", "cookie", "token"):
...
def _write_markdown_report(path: Path, summary: dict[str, Any]) -> None:
    lines = [
        "# Phase4-AZ Long History Controlled Fetch Retry",
        "",
        f"- status: `{summary.get('status')}`",
        f"- readiness_status: `{summary.get('readiness_status')}`",
        f"- corrected range: `{summary.get('corrected_fetch_start_date')}` to `{summary.get('fetch_end_date')}`",
        f"- corrected/executed/succeeded/failed/skipped/completed: `{summary.get('corrected_request_count')}` / `{summary.get('executed_request_count')}` / `{summary.get('succeeded_request_count')}` / `{summary.get('failed_request_count')}` / `{summary.get('skipped_request_count')}` / `{summary.get('completed_request_count')}`",
        f"- pagination_request_count: `{summary.get('pagination_request_count')}`",
        f"- pre_start_request_executed: `{summary.get('pre_start_request_executed')}`",
        f"- quarantined_failed_manifest_count: `{summary.get('quarantined_failed_manifest_count')}`",
...
def _print_human_summary(summary: dict[str, Any]) -> None:
    print("Phase4-AZ long history controlled fetch retry")
    print(f"status={summary.get('status')}")
    print(f"readiness_status={summary.get('readiness_status')}")
    print(f"corrected_date_range={summary.get('corrected_fetch_start_date')}..{summary.get('fetch_end_date')}")
    print(f"corrected_request_count={summary.get('corrected_request_count')}")
    ...
    print(f"pre_start_request_executed={summary.get('pre_start_request_executed')}")
