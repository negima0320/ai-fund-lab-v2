...
PHASE = "Phase4-AJ"
SUMMARY_PATH = Path("reports/candidate_ai/full_range/phase4aj_real_runtime_normalized_summary.json")
PHASE4AI_SUMMARY_PATH = Path("reports/candidate_ai/full_range/phase4ai_post_extension_raw_coverage_summary.json")

READY_INPUT = "READY_FOR_REAL_RUNTIME_NORMALIZED_REBUILD"
READY = "READY_FOR_REAL_RUNTIME_FEATURE_GENERATION"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Rebuild isolated real_runtime daily_quotes_normalized data from raw J-Quants response files."
    )
    ...
    parser.add_argument("--phase4ai-summary", default=str(PHASE4AI_SUMMARY_PATH))
    ...
    summary = rebuild_phase4aj_real_runtime_normalized(
        ...
        phase4ai_summary_path=Path(args.phase4ai_summary),
    )
    print(json.dumps(summary, ensure_ascii=True, indent=2, sort_keys=True))
    return 0 if summary.get("status") in {"OK", "BLOCKED"} else 1


def rebuild_phase4aj_real_runtime_normalized(
    *,
    runtime_dir: Path | str = ".runtime",
    report_dir: Path | str = "reports/candidate_ai/full_range",
    output_format: str = "parquet",
    phase4ai_summary_path: Path = PHASE4AI_SUMMARY_PATH,
) -> dict[str, Any]:
    ...
    phase4ai_summary = _read_json_optional(phase4ai_summary_path)

    if phase4ai_summary.get("readiness_status") != READY_INPUT:
        summary = _blocked_summary(
            readiness_status=BLOCKED_RAW_SCHEMA,
            reason="Phase4-AI summary is missing or not ready for normalized rebuild.",
            paths=paths,
    ...
    if not raw_records:
        summary = _blocked_summary(
            readiness_status=BLOCKED_RAW_SCHEMA,
            reason="No raw daily quote records were found in response files.",
            paths=paths,
            report_dir=report_dir,
            response_dir=response_dir,
            raw_manifest_path=raw_manifest_path,
    ...
    manifest = _build_manifest(
        ...
        raw_manifest_path=raw_manifest_path,
        phase4ai_summary_path=phase4ai_summary_path,
        isolated_output_path=isolated_output_path,
    ...
    summary = {
        ...
        "phase4ai_summary_path": str(phase4ai_summary_path),
...
def _resolve_readiness(
    *,
    ...
    if business_day_count < REQUIRED_BUSINESS_DAY_COUNT or not mock_path_unchanged:
        return BLOCKED_OUTPUT_PATH
...
def _blocked_summary(
    *,
    ...
    return {
        ...
        "order_executed": False,
        "portfolio_auto_update_executed": False,
        "recommended_next_action": "Fix the blocking condition, then rerun Phase4-AJ normalized rebuild.",
