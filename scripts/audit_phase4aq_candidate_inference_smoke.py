...
from scripts.run_phase4aq_candidate_inference_smoke import (  # noqa: E402
    READY,
    SUMMARY_PATH,
    TOP50_CSV_PATH,
    TOP50_JSON_PATH,
...
JSON_REPORT_PATH = Path("reports/phase_reports/phase4aq_candidate_inference_smoke_audit.json")
MARKDOWN_REPORT_PATH = Path("docs/phase_reports/phase4aq_candidate_inference_smoke_audit.md")
...
def run_audit(
    *,
    ...
    if not summary or not top50_json_path.is_file():
        summary = run_phase4aq_candidate_inference_smoke(runtime_dir=runtime_dir, report_dir=report_dir)
    ...
    checks = {
        ...
        "readiness_ready_for_candidate_output_audit_smoke": summary.get("readiness_status") == READY,
        ...
        "score_stats_recorded": all(summary.get(key) is not None for key in ("candidate_score_min", "candidate_score_max", "candidate_score_mean")),
    ...
    result = {
        ...
        "pytest_hint": "python3 -m pytest tests/test_phase4aq_candidate_inference_smoke.py && python3 -m pytest -q",
    }
