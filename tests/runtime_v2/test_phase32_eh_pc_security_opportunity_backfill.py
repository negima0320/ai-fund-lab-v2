...
def _run_backfill(*args: str) -> subprocess.CompletedProcess[str]:
    env = dict(os.environ)
    env["PYTHONPATH"] = "src:."
    env["PYTHONPYCACHEPREFIX"] = "/private/tmp/pycache-phase32-eh-tests"
    return subprocess.run(
        [sys.executable, str(RUNTIME_TEST), "shadow-backfill-pc-security-opportunity", *args, "--json"],
        cwd=REPO_ROOT,
        env=env,
        text=True,
        capture_output=True,
...
def _minimal_pc(date: str) -> dict:
    ...
    return {
        ...
        "capital_competition": {
            "competitors": [
                {"accepted_weight": 0.0, "competitor_type": "ADD", "status": "REJECTED", "symbol": "94320"},
                {"accepted_weight": 0.1, "competitor_type": "NEW_BUY", "status": "COMPETITOR_SELECTED", "symbol": "38520"},
            ],
...
def test_phase32_eh_pc_security_opportunity_backfill_dry_run_is_non_mutating(tmp_path: Path) -> None:
    ...
    assert payload["summary"]["pc_security_opportunity_shadow_consumer"] == "PASS"
    assert payload["summary"]["authoritative_consumer_count"] == 0
    assert not output_root.exists()


def test_phase32_eh_pc_security_opportunity_backfill_writes_analysis_only_consumer(tmp_path: Path) -> None:
    evidence_root = tmp_path / "runtime_tests"
    run_id = "runtime-test-source"
    _write_run(evidence_root, run_id, "2023-02-13", _minimal_pc("2023-02-13"))
    ...
    daily = json.loads(
        (output_root / "daily" / "2023-02-13" / "pc_security_opportunity_shadow_consumer.json").read_text(
            encoding="utf-8"
        )
    )
    assert summary["production_change_executed"] is False
    assert summary["add_unknown_116_pc_shadow_reclassification"]["COMPARABLE_NEGATIVE"] == 1
    consumer = daily["pc_security_opportunity_shadow_consumer"]
