...
def test_phase24_ht_planning_exposure_fail_blocks_approved_pending(tmp_path: Path) -> None:
    ...
    assert linked.planning_submit_feasibility["items"][0]["violated_policy"] == "max_exposure"
    assert linked.planning_submit_feasibility["items"][0]["remaining_exposure"] == 164_490
...
def test_phase24_id_planning_aggregate_cash_reservation_blocks_later_buy(tmp_path: Path) -> None:
    ...
    pending = _pending(
        (
            _item("buy-1", amount=150_000, symbol="7203"),
            _item("buy-2", amount=150_000, symbol="6758"),
        ),
        policy=policy,
    )
    ...
    assert linked.planning_submit_feasibility["items"][0]["post_buy_cash"] == 100_000
    assert linked.planning_submit_feasibility["items"][1]["status"] == "REVIEW_REQUIRED"
    assert linked.planning_submit_feasibility["items"][1]["cash"] == 100_000
    assert linked.planning_submit_feasibility["items"][1]["violated_policy"] == "cash"
    assert linked.approved_item_ids == ()
    assert linked.approved_buy_item_ids == ()
    assert linked.review_required_buy_item_ids == ("buy-2",)
    assert linked.items[0].approved is False
    ...
    assert linked.items[1].feasibility_status == "REVIEW_REQUIRED"
    assert linked.items[1].batch_submit_status == "ITEM_REVIEW_REQUIRED"
...
def test_phase24_id_submit_aggregate_preflight_blocks_before_adapter_boundary(tmp_path: Path) -> None:
    ...
    policy_path = _write_policy(tmp_path / "capital_deployment_policy.json")
    policy = load_capital_deployment_policy(policy_path)
    _write_current(root, cash=600_000, positions=[_position("1111", 100, 1000)])
    pending = _pending(
        (
    ...
    _write_current(root, cash=300_000, positions=[_position("1111", 100, 1000)])
    ...
    assert result.reason == "submit aggregate feasibility failed before broker boundary"
    blocked = [item for item in result.submit_guard_item_evidence if item["guard_decision"] == "BLOCKED"]
    assert blocked
    assert blocked[0]["violated_policy"] in {"cash", "buying_power"}
...
def test_phase24_ht_planning_pass_then_submit_guard_passes(tmp_path: Path) -> None:
    ...
    assert result.submit_guard_item_evidence[0]["guard_decision"] == "PASS"
...
def test_phase24_ht_submit_guard_revalidates_after_planning_pass(tmp_path: Path) -> None:
    ...
    _write_current(root, cash=500_000, positions=[_position("1111", 1, 800_000)])
    write_pending_order_plan(root / "pending_order_plan" / "pending_order_plan.json", pending)
    ...
    assert result.submit_guard_item_evidence[0]["violated_policy"] == "max_exposure"
