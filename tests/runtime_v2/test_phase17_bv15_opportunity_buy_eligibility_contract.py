...
def test_phase17_bv15_morning_excludes_36810_negative_edge_from_pending(tmp_path: Path) -> None:
    runtime_root = _historical_runtime_root(tmp_path / ".runtime")
    feature_root = _feature_root(runtime_root, FEATURE_DATE)
    _append_price_rows(runtime_root, symbols=("36810",))
    ...
    opportunity_path = _write_opportunity_artifact(
        ...
        [
            _opportunity_row("36810", expected_edge=-0.06934237, no_buy_reason="non_positive_expected_edge_score", buy_rank=2),
            _opportunity_row("7203", expected_edge=0.08, no_buy_reason="", buy_rank=7),
        ],
    )
    ...
    result = run_morning_ai_planning_pending_pipeline(
        ...
        ai_signals=(
            AIPlanningSignal("signal-36810", "36810", "BUY", 2, -0.06934237, "opportunity_top5", "opportunity_ai"),
            AIPlanningSignal("signal-7203", "7203", "BUY", 7, 0.08, "opportunity_ranked", "opportunity_ai"),
        ),
        buy_ai_context={"opportunity_artifact_path": str(opportunity_path)},
        safety_decision=replace(_historical_safety(), business_date=BUSINESS_DATE),
    ...
    assert result.opportunity_buy_eligibility_filtered_count == 1
    assert {item["symbol"] for item in pending["items"]} == {"7203"}
    assert order_plan["opportunity_buy_eligibility_contract"]["filtered_count"] == 1
    assert pending["items"][0]["listed_info"]["opportunity_buy_eligibility"] == "BUY_ELIGIBLE"


def test_phase17_bv15_all_negative_opportunities_create_no_buy_pending(tmp_path: Path) -> None:
    runtime_root = _historical_runtime_root(tmp_path / ".runtime")
    feature_root = _feature_root(runtime_root, FEATURE_DATE)
    _append_price_rows(runtime_root, symbols=("36810", "33500"))
    policy_path = _write_policy(tmp_path / "capital_deployment_policy.json")
    opportunity_path = _write_opportunity_artifact(
        runtime_root / "runtime_state" / "buy_ai" / BUSINESS_DATE / "opportunity_rankings.json",
        [
            _opportunity_row("36810", expected_edge=-0.06934237, no_buy_reason="non_positive_expected_edge_score", buy_rank=2),
            _opportunity_row("33500", expected_edge=0.0, no_buy_reason="non_positive_expected_edge_score", buy_rank=1),
        ],
    )
    ...
    result = run_morning_ai_planning_pending_pipeline(
        ...
        capital_deployment_policy_path=policy_path,
        ai_signals=(
            AIPlanningSignal("signal-33500", "33500", "BUY", 1, 0.0, "opportunity_top5", "opportunity_ai"),
            AIPlanningSignal("signal-36810", "36810", "BUY", 2, -0.06934237, "opportunity_top5", "opportunity_ai"),
        ),
        buy_ai_context={"opportunity_artifact_path": str(opportunity_path)},
        safety_decision=replace(_historical_safety(), business_date=BUSINESS_DATE),
    ...
    assert result.opportunity_buy_eligibility_filtered_count == 2
