...
def test_phase17_bv10_historical_partial_sell_execution_projects_remaining_quantity(tmp_path: Path) -> None:
    runtime_root, policy_path = _sell_fixture(tmp_path, owned_quantity=250.0, sell_quantity=100.0)
    ...
    assert state["positions"][0]["quantity"] == 150.0
    assert state["cash"] == 1_300_000.0
    assert result.reconcile_status == "PASS"


def test_phase17_bv10_duplicate_historical_execution_is_idempotent(tmp_path: Path) -> None:
    runtime_root, policy_path = _sell_fixture(tmp_path, owned_quantity=100.0, sell_quantity=100.0)
    adapter = _adapter(tmp_path, runtime_root)
