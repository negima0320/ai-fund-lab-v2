...
def test_report_notification_audit_do_not_import_side_effect_modules():
...
def test_report_notification_audit_do_not_call_side_effect_helpers():
    for path in _runtime_files():
        tree = ast.parse(path.read_text(encoding="utf-8"))
        for node in ast.walk(tree):
...
def test_report_notification_audit_do_not_reference_forbidden_writers_or_scheduler_text():
    for path in _runtime_files():
...
def _runtime_files():
    roots = (
        Path("src/ai_fund_lab_v2/runtime_v2/report"),
        Path("src/ai_fund_lab_v2/runtime_v2/notification"),
        Path("src/ai_fund_lab_v2/runtime_v2/audit"),
    )
    files = []
    for root in roots:
