...
def test_phase9s_daily_continuation_updates_valuation_and_reports_without_virtual_fill(tmp_path: Path) -> None:
    ...
    assert result.status == DAILY_CONTINUATION_COMPLETED
    assert result.ledger_latest_updated is True
    assert latest.performance.total_equity == Decimal("993140.0")
    assert result.position_management_input_count == 5
    assert result.pending_order_count_after == 0
    assert result.created_pending_order_count == 0
    assert result.virtual_fill_executed is False
    assert Path(result.performance_report_json_path).is_file()
    assert Path(result.public_performance_summary_path).is_file()
    assert any("no_virtual_fill" in warning for warning in result.warnings)
