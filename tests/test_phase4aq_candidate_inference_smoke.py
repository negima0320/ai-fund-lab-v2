...
from scripts.audit_phase4aq_candidate_inference_smoke import run_audit
from scripts.run_phase4aq_candidate_inference_smoke import (
    READY,
    audit_inference_features,
    run_phase4aq_candidate_inference_smoke,
    validate_candidate_output,
)


def test_phase4aq_runs_candidate_inference_smoke(tmp_path: Path) -> None:
    runtime_dir, ap_summary, ak_summary, an_summary = _prepare_fixture(tmp_path, row_count=60)
    ...
    assert summary["target_date"] == "2026-05-29"
...
def test_phase4aq_top50_schema_and_sort(tmp_path: Path) -> None:
    runtime_dir, ap_summary, ak_summary, an_summary = _prepare_fixture(tmp_path, row_count=60)
    summary = run_phase4aq_candidate_inference_smoke(
        runtime_dir=runtime_dir,
        report_dir=tmp_path / "reports",
...
def test_phase4aq_feature_audit_rejects_label_and_future_features() -> None:
...
def test_phase4aq_audit_completes(tmp_path: Path) -> None:
    runtime_dir, ap_summary, ak_summary, an_summary = _prepare_fixture(tmp_path, row_count=60)
    summary_path = tmp_path / "reports" / "phase4aq_candidate_inference_smoke_summary.json"
    run_phase4aq_candidate_inference_smoke(
        runtime_dir=runtime_dir,
        report_dir=tmp_path / "reports",
        phase4ap_summary_path=ap_summary,
...
def test_phase4aq_report_documents_inference_smoke_scope() -> None:
    report = Path("docs/phase_reports/phase4aq_candidate_inference_smoke.md").read_text(encoding="utf-8")

    assert "Candidate Inference Smoke" in report
    assert "READY_FOR_CANDIDATE_OUTPUT_AUDIT_SMOKE" in report
    assert "not a buy signal" in report
    assert "backtest" in report


def _prepare_fixture(tmp_path: Path, *, row_count: int) -> tuple[Path, Path, Path, Path]:
    ...
    model = LogisticRegression(random_state=42).fit(
        np.asarray([[0.1, 1.5, 100000], [-0.1, 0.5, 10000], [0.2, 1.8, 200000], [-0.2, 0.7, 20000]], dtype=float),
        np.asarray([1, 0, 1, 0], dtype=int),
    )
    model_path = model_dir / "phase4ap_candidate_smoke_model.pkl"
    manifest_path = model_dir / "phase4ap_candidate_smoke_manifest.json"
    ...
    feature_path = feature_dir / "features.json"
    feature_path.write_text(json.dumps({"rows": [_feature_row(index) for index in range(row_count)]}), encoding="utf-8")
    ap_summary = tmp_path / "phase4ap.json"
    ...
    ak_summary.write_text(json.dumps({"feature_output_path": str(feature_path)}), encoding="utf-8")
    an_summary.write_text(json.dumps({"historical_feature_output_path": str(feature_path)}), encoding="utf-8")
    return runtime_dir, ap_summary, ak_summary, an_summary
...
def _feature_row(index: int) -> dict[str, object]:
    ...
    return {
        ...
        "liquidity_avg_volume_20d": 200000 - index * 100,
    }
