"""Phase14-D5 pure Runtime v2 Demo BUY retest harness."""
...
def run_phase14d5_pure_runtime_v2_demo_buy_retest(
    *,
    root: Path,
    ...
    before_status = _readonly_snapshot(
        ...
        symbol=symbol,
        source="phase14d5_readonly_before",
    )
    if before_status not in {"PASS", "PASS_WITH_WARNINGS"}:
    ...
    approval_artifact_path = root / "approval_artifact" / "approval_phase14d5_demo_buy.json"
    ...
    if submit_result and submit_result.broker_api_called:
        after_status = _readonly_snapshot(
            ...
            symbol=symbol,
            source="phase14d5_readonly_after",
        )
        if after_status not in {"PASS", "PASS_WITH_WARNINGS"}:
    ...
    if submit_result and submit_result.broker_api_called and ledger_orders:
        ...
        pending_plan = consume_pending_plan(
            pending_plan,
            consume_reason="phase14d5 pure runtime v2 demo buy submit attempted",
    ...
    asset_state = build_current_asset_state(
        ...
        source="phase14d5_demo_broker_readonly",
    ...
    notification_payload = build_notification_payload(report=report, channel="phase14d5_payload_only")
    audit = run_audit(
        mode="demo",
        environment="demo",
        business_date=_business_date(),
    ...
    execution_classification = "FILLED" if broker_executions else "UNFILLED_OR_NOT_CONFIRMED"
    final_decision = "PHASE14D5_PURE_RUNTIME_V2_DEMO_BUY_PASS"
    if blocked_reasons or review_reasons or not (submit_result and submit_result.accepted):
        final_decision = "PHASE14D5_REVIEW_REQUIRED"
    result = Phase14D5Result(
        ...
        order_status_readonly_confirmed=bool(broker_orders),
...
def _build_pending_and_approval(*, symbol: str, quantity: float, estimated_price: float, root: Path) -> tuple[PendingOrderPlan, ApprovalArtifact]:
    item = PendingOrderItem(
        pending_item_id=f"phase14d5-buy-{symbol}-{int(quantity)}",
        symbol=symbol,
        side="BUY",
        quantity=quantity,
        order_type="MARKET",
    ...
    plan = promote_order_plan_to_pending(
        order_plan_id=f"phase14d5-demo-buy-{symbol}",
        source_order_plan_path="order_plan/phase14d5-demo-buy.json",
        source_order_plan_hash=_hash_text(f"phase14d5-demo-buy-{symbol}"),
        environment="demo",
    ...
    approval = ApprovalArtifact(
        approval_id="approval_phase14d5_demo_buy",
        approval_request_id="approval_request_phase14d5_demo_buy",
        ...
        reason=f"Phase14-D5 manual approval for pure Runtime v2 Demo BUY {symbol}",
    )
...
def _bundle_from_snapshot(path: Path):
    if not path.exists():
        return normalize_broker_readonly_payload(
            environment="demo",
            source="phase14d5_missing_snapshot",
            as_of=_business_date(),
...
def _markdown_report(result: Phase14D5Result) -> str:
    return f"""# Phase14-D5 Pure Runtime v2 Demo BUY Single-Order Re-test
