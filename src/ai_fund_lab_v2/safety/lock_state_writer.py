...
def write_unlock_applied_state(
    result: UnlockApplyResult,
    runtime_dir: Path | str = ".runtime",
) -> Path:
    path = Path(runtime_dir) / "safety" / "locks" / f"unlock_applied_{_request_id(result)}_{_file_timestamp()}.json"
...
def _safe_payload(value: Any) -> dict[str, Any]:
    if not is_dataclass(value):
        raise TypeError("lock state writer expects a dataclass payload")
