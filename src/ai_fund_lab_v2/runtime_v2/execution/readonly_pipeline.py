    ...
    for order in orders_tuple:
        ...
        records.append(
            LedgerExecutionRecord(
                ...
                order_plan_item_id=order.order_plan_item_id,
                position_campaign_id=order.position_campaign_id,
                campaign_id=order.campaign_id or order.position_campaign_id,
    ...
    for execution in executions:
        ...
        records.append(
            LedgerPositionRecord(
                ...
                campaign_id=execution.campaign_id or execution.position_campaign_id,
            )
        )
...
def _runtime_order_payload(order: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "source_decision_id": order.get("source_decision_id") or "",
        "source_decision_type": order.get("source_decision_type") or "",
        "source_pm_decision_id": order.get("source_pm_decision_id") or "",
        "order_plan_item_id": order.get("order_plan_item_id") or "",
        "position_campaign_id": order.get("position_campaign_id") or order.get("campaign_id") or "",
        "campaign_id": order.get("campaign_id") or order.get("position_campaign_id") or "",
