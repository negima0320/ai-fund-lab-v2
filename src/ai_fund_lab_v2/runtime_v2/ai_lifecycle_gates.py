...
def evaluate_freshness_gate(evidence: dict[str, Any], thresholds: FreshnessThresholds | None = None) -> RuntimeAIGateResult:
    ...
    for code in evidence.get("reason_codes") or []:
        status = "BLOCK" if any(token in str(code) for token in ("negative_", "future", "calendar_", "weekday_fallback")) else "REVIEW_REQUIRED"
...
def evaluate_drift_gate(evidence: dict[str, Any], thresholds: DriftThresholds | None = None) -> RuntimeAIGateResult:
    ...
    if baseline_scores and current_scores:
        ...
        status = "BLOCK" if psi > thresholds.psi_block else "REVIEW_REQUIRED" if psi > thresholds.psi_review else "PASS"
    ...
    if baseline_feature and current_feature:
        feature_psi = population_stability_index(baseline_feature, current_feature)
        status = "BLOCK" if feature_psi > thresholds.psi_block else "REVIEW_REQUIRED" if feature_psi > thresholds.psi_review else "PASS"
    ...
    if baseline_positive is not None and current_positive is not None:
        ...
        status = "BLOCK" if ratio < thresholds.positive_rate_block_ratio else "REVIEW_REQUIRED" if ratio < thresholds.positive_rate_review_ratio else "PASS"
    ...
    if baseline_population is not None:
        ...
        pop_status = "BLOCK" if pop_ratio < 0.10 else "REVIEW_REQUIRED" if pop_ratio < 0.25 else "PASS"
    ...
    status = "BLOCK" if all_negative_days >= thresholds.all_negative_block_days else "REVIEW_REQUIRED" if all_negative_days >= thresholds.all_negative_review_days else "PASS"
...
def _compose_result(checks: list[GateEvidence], *, market_no_opportunity: bool = False) -> RuntimeAIGateResult:
    ...
    if "BLOCK" in statuses:
        decision = "BLOCK"
        state = "CRITICAL_AUTHORITY_VIOLATION" if any("critical authority" in item.reason for item in checks if item.status == "BLOCK") else "MODEL_UNHEALTHY"
        buy_gate = "BUY_BLOCK"
        block_buy = True
        block_submit = True
