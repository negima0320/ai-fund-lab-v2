...
class HistoricalSubmitAdapter:
    ...
    def submit(self, command: RuntimeV2SubmitCommand) -> RuntimeV2SubmitResult:
        ...
        evidence = {
            ...
            "campaign_id": command.campaign_id or command.position_campaign_id,
...
def _order_payload(item: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "source_decision_id": item.get("source_decision_id") or "",
        "source_decision_type": item.get("source_decision_type") or "",
        "source_pm_decision_id": item.get("source_pm_decision_id") or "",
        "order_plan_item_id": item.get("order_plan_item_id") or "",
        "position_campaign_id": item.get("position_campaign_id") or "",
        "campaign_id": item.get("campaign_id") or item.get("position_campaign_id") or "",
