...
Phase32-DL adds the canonical operator resolution path for per-symbol Corporate Action Adjustment Authority. The path reuses `runtime_v2_corporate_action_adjustment_authority_v1`; it does not introduce a Historical bypass and does not infer event type or adjusted quantity from `AdjFactor` alone. An operator-reviewed resolution may transition the authority to `PASS` only when it records run/date/symbol binding, reviewer and audit id, manually supplied event type, effective date, factor, pre/post quantity evidence, adjusted Runtime-owned quantity, price-basis reconciliation, ledger/current/pending adjustment statuses, already-applied idempotency confirmation, original unresolved artifact path/hash, reviewed evidence sources, timestamp, and future-information status. Submit remains blocked when Pending or Submit quantity exceeds the adjusted owned/broker-available quantity, when adjusted quantity differs from stale Pending authority and regeneration is required, or when source/run binding, PIT hash, double-adjustment, reviewer, or reconciliation evidence is missing.

SELL campaign identity is part of the same Planning-to-Pending authority boundary. BUY_NEW may materialize a new campaign once; SELL_EXIT and SELL_REDUCE must inherit the current open position campaign from explicit upstream evidence or the same-run position campaign artifact. A deterministic replacement campaign must not be generated for a SELL. If explicit SELL campaign identity conflicts with the current open campaign, or if no canonical current campaign can be proven, the item fails closed before Pending approval instead of submitting under an empty or regenerated campaign id.
